...
export default function AddToPlaylistModal({ song, onClose }: Props) {
  ...
  const handleAdd = async (playlistId: string) => {
    ...
    if (res.success) {
      ...
      setMessage(res.message || "Không thể thêm (có thể đã tồn tại)");
    }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-surface-elevated rounded-2xl w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold">Thêm vào danh sách phát</h3>
        ...
        {message ? (
          ...
          <>
            <div className="space-y-1 max-h-52 overflow-y-auto mb-4">
              {playlists.length === 0 ? (
                <p className="text-text-muted text-xs text-center py-3">Chưa có playlist nào</p>
              ) : (
                playlists.map((pl) => (
                  <button key={pl.id} onClick={() => handleAdd(pl.id)}
                    ...
                    <span className="text-xs text-text-muted ml-auto shrink-0">{pl.songs.length} bài</span>
                  </button>
                ))
              )}
            </div>
            ...
            {creating ? (
              <form onSubmit={handleCreate} className="flex gap-2">
                <input autoFocus type="text" placeholder="Tên playlist mới..."
